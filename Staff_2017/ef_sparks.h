...
public:
  ...
  static const int numsparks = 304;
  ...
  {
    // render the spark as a span of pixels starting with the first color in
    // the palette and covering the entire palette over the width of the span
    // Don't allow rendering to wrap around the end of the staff. Clamp at 0,
    // NUM_LEDS/2 and NUM_LEDS
    // Later, add some methods for specifying render direction and mirroring
    // options
