...
class effect
{
  public:
	...
	virtual void SetBuffer(CRGB* dest) {dst = dest;}; // Must have a destination to render
	virtual CRGB* GetBuffer() {return dst;}; // Get the working buffer for this effect
    ...
    virtual void SetBrightness(uint8_t b) {brightness = b;}
    ...
    virtual void SetPal(uint8_t p) {pal = p;};
